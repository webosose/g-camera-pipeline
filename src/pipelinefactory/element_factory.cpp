...
std::string ElementFactory::GetPreferredElementName(const std::string &pipelineType, const std::string & elementTypeName)
{
    ...
    for(const auto& elements : gstElements.items()) {
        if (elements.hasKey("pipeline-type")
            ...
            if (elements.hasKey(elementTypeName)
              && elements[elementTypeName].hasKey("name")) {
                elementName = elements[elementTypeName]["name"].asString();
                CMP_LOG_INFO("[%s] %s: %s", pipelineType.c_str(), elementTypeName.c_str(), elementName.c_str());
            } else {
                CMP_LOG_INFO("elementTypeName: %s is not exist",
...
void ElementFactory::SetProperties(const std::string &pipelineType, GstElement * element, const std::string &elementTypeName)
{
    pbnjson::JValue root = pbnjson::JDomParser::fromFile(gst_element_json_path.c_str());
    if (!root.isObject()) {
        CMP_LOG_INFO("Gst element file parsing error");
...
void ElementFactory::SetProperty(GstElement * element,
    ...
    if (objProp.isString()) {
        ...
        if (objValue.isNumber()) {
            ...
            CMP_LOG_INFO("[%s] %s: %s", elementName, strProp.c_str(), objValue.asBool() ? "true" : "false");
