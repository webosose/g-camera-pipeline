...
BufferEncoder::BufferEncoder()
    : bus_(nullptr), load_complete_(false), pipeline_(nullptr), source_(nullptr),
      filter_YUY2_(nullptr), parse_(nullptr), converter_(nullptr), filter_NV12_(nullptr),
      encoder_(nullptr), sink_(nullptr), caps_YUY2_(nullptr),
      caps_NV12_(nullptr), encdata_{FALSE, 0, 0, 0, 0, 0, nullptr, CMP_VIDEO_CODEC_NONE},
      userData(nullptr)
{
...
bool BufferEncoder::GetSourceInfo(const ENCODER_INIT_DATA_T *loadData)
{
    ...
    CMP_DEBUG_PRINT("[video info] width: %d, height: %d, frameRate: %d/%d", video_stream_info.width,
...
void BufferEncoder::Notify(const gint notification, const gint64 numValue, const gchar *strValue,
                           void *payload)
{
...
bool BufferEncoder::init(const ENCODER_INIT_DATA_T *loadData)
{
    ...
    if (loadData->codecFormat == CMP_VIDEO_CODEC_H264)
    {
        ...
        resourceRequestor_ = std::make_unique<cmp::resource::ResourceRequestor>(app_id_, media_id_);
        ...
        if (resourceRequestor_)
        {
            if (!resourceRequestor_->acquireResources(resourceMMap, source_info_, display_mode_,
                                                      display_path_))
            {
 ...
 * processing */
GstFlowReturn BufferEncoder::on_new_sample_from_sink(GstElement *elt, ProgramData *data)
{
    CMP_INFO_PRINT("%d %s", __LINE__, __FUNCTION__);
...
int BufferEncoder::feed(const uint8_t *yBuffer, const uint8_t *uBuffer, const uint8_t *vBuffer,
                        guint32 bufferSize)
{
    if (!pipeline_)
    {
...
bool BufferEncoder::SendBackEncodedData(uint8_t *buffer, ENCODED_BUFFER_T *encData)
{
    bool res;
...
{
    CMP_INFO_PRINT("%d %s, width: %d, height: %d", __LINE__, __FUNCTION__, loadData->width,
                   loadData->height);
    ...
    caps_YUY2_ = gst_caps_new_simple("video/x-raw", "width", G_TYPE_INT, loadData->width, "height",
                                     G_TYPE_INT, loadData->height, "framerate", GST_TYPE_FRACTION,
                                     loadData->frameRate, 1, "format", G_TYPE_STRING, "I420", NULL);
...
gboolean BufferEncoder::HandleBusMessage(GstBus *bus_, GstMessage *message, gpointer user_data)
{
    ...
    {
        CMP_INFO_PRINT("Element[ %s ][ %d ][ %s ]", GST_MESSAGE_SRC_NAME(message), messageType,
    ...
    case GST_MESSAGE_STATE_CHANGED:
    {
        ...
        CMP_INFO_PRINT("Element[%s] State changed ...%s -> %s", GST_MESSAGE_SRC_NAME(message),
