...
Service::Service(const char *service_name)
    : media_id_(""), app_id_(""), umc_(nullptr), player_(nullptr), resourceRequestor_(nullptr),
      isLoaded_(false)
{
...
void Service::Notify(const gint notification, const gint32 numValue, const gchar *strValue,
                     void *payload)
{
...
bool Service::LoadEvent(UMSConnectorHandle *handle, UMSConnectorMessage *message, void *ctxt)
{
    ...
    if (parsed.hasKey("args") && parsed["args"].isArray())
    {
        ...
        {
            if (parsed["args"][i]["option"].hasKey("appId") &&
                parsed["args"][i]["option"]["appId"].isString())
            {
...
bool Service::TakeCameraSnapshotEvent(UMSConnectorHandle *handle, UMSConnectorMessage *message,
                                      void *ctxt)
{
    std::string msg = instance_->umc_->getMessageText(message);
    CMP_LOG_INFO("message : %s", msg.c_str());
...
bool Service::StartCameraRecordEvent(UMSConnectorHandle *handle, UMSConnectorMessage *message,
                                     void *ctxt)
{
    pbnjson::JDomParser jsonparser;
    ...
    return instance_->player_->StartRecord(parsed["location"].asString(),
                                           parsed["format"].asString(), parsed["audio"].asBool(),
                                           parsed["audioSrc"].asString());
}

bool Service::StopCameraRecordEvent(UMSConnectorHandle *handle, UMSConnectorMessage *message,
                                    void *ctxt)
{
...
void Service::LoadCommon()
{
    ...
    player_->RegisterCbFunction(std::bind(&Service::Notify, instance_, std::placeholders::_1,
...
bool Service::AcquireResources(const base::source_info_t &sourceInfo,
                               const std::string &display_mode, const int32_t display_path)
{
